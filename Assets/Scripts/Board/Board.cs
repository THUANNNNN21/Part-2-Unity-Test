...
public class Board
{
    ...
    public Board(Transform transform, GameSettings gameSettings, int customSizeX, int customSizeY)
    {
        m_root = transform;

        m_matchMin = gameSettings.MatchesMin;

        this.boardSizeX = customSizeX;
        this.boardSizeY = customSizeY;
    ...
    public void Clear()
    {
        if (m_cells == null) return; // Kiểm tra array null
        ...
        for (int x = 0; x < boardSizeX; x++)
        {
            for (int y = 0; y < boardSizeY; y++)
            {
                ...
                if (cell == null) continue; // Bỏ qua cell null
    ...
    // Lấy tất cả cells dưới dạng array (từ trái qua phải, dưới lên trên)
